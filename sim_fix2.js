...
async function run() {
  const popupPub = '300x250-0xb9a3faeb416580f4bc1c8f6e2d4773b580e9d18c';
  const floatPub = '64x64-0xb9a3faeb416580f4bc1c8f6e2d4773b580e9d18c';

  const iframePopup = `<iframe src="https://openads-backend.vercel.app/serve?placement=${popupPub}&position=popup&preview=true" style="position:fixed; top:50%; left:50%; transform:translate(-50%, -50%); z-index:999999; border:none; box-shadow: 0 20px 40px rgba(0,0,0,0.3); border-radius: 8px; background: #fff;" width="300" height="250" frameborder="0" scrolling="no"></iframe>`;
  ...
  fs.writeFileSync('sim_mobile2.html', mobileHtml);
  fs.writeFileSync('sim_desktop2.html', desktopHtml);
  ...
  await page.goto('file://' + __dirname + '/sim_mobile2.html', { waitUntil: 'load' });
  ...
  await page.goto('file://' + __dirname + '/sim_desktop2.html', { waitUntil: 'load' });
  await new Promise(r => setTimeout(r, 6000));
